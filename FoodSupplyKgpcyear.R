...
fs.fun <- function(proj.kgpc){
  ...
  # ignore the warning, it is generated by factors
...
ggsave("Annual supply Kg pc 80% pred interval.tiff" , 
       plot = hp.kgpc, 
       dpi = 400, width = 300, 
       height = 350, 
       units = "mm",
...
for (pop.scen in all.scen){
  ...
  hist.cons$protein.kgpc = hist.cons$hist.tonnes * hist.cons$proteingrams.per.gram
  hist.cons$fat.kgpc = hist.cons$hist.tonnes * hist.cons$fatgrams.per.gram
  ...
  trend.tonnesL$protein.kgpc = trend.tonnesL$hist.tonnes * trend.tonnesL$proteingrams.per.gram
  trend.tonnesL$fat.kgpc = trend.tonnesL$hist.tonnes * trend.tonnesL$fatgrams.per.gram
  
  indicators = c("Year", "ItemC", "hist.tonnes", "protein.kgpc", "fat.kgpc")
  food.supply = rbind(hist.cons[,indicators], trend.tonnesL[,indicators])
  food.supply = merge(food.supply, pop.hp, by = "Year", all.x = T)
  food.supply = merge(food.supply, categories[, c("ItemC", "kcals.per.gram")], by = "ItemC", all.x = T)
  food.supply = food.supply[order(food.supply$ItemC,food.supply$Year), ]
  food.supply$consumption.kcals = food.supply$hist.tonnes * gr2ton * food.supply$kcals.per.gram
  colnames(food.supply) = c("ItemC", "Year", "consumption.tonnes", "protein.tonnes", "fat.tonnes", "population",
                           "kcals.per.gram",  "consumption.kcals")
  
  hp.tonnes <- ggplot(food.supply, aes(x=Year, y = consumption.tonnes/1000)) + 
    ylab("1000 Tonnes") +
    scale_x_continuous(breaks=c(1961, 1980, 2000, 2020, 2040, 2060)) +
    geom_line() + 
    facet_wrap( ~ ItemC, ncol=4, scales="free_y") +
  ...
  write.csv(food.supply, paste("historical and projected food supply", pop.scen, ".csv"), row.names = F)
}
